...
namespace rs
{
    ...
    public:
        ...
        void on_event(rs_motion_data e)
        {
    ...
    public:
        ...
        void on_event(rs_timestamp_data data)
        {
    ...
    public:
        ...
        void on_frame(rs_device * device, rs_frame_ref * fref)
        {
    ...
    public:
        ...
        void get_stream_mode(stream stream, int index, int & width, int & height, int & format, int & framerate) const
        {
            rs_error * e = nullptr;
            rs_format f;
            rs_get_stream_mode((const rs_device *)this, (rs_stream)stream, index, &width, &height, &f, &framerate, &e);
            error::handle(e);
        ...
        {
            rs_error * e = nullptr;
            rs_set_frame_callback_cpp((rs_device *)this, (rs_stream)stream,(rs_frame_callback*) new frame_callback(frame_handler), &e);
            error::handle(e);
        }
        ...
        bool supports_capability(capabilities capability) const
        {
            rs_error * e = nullptr;
        ...
        /// \param[in] capability  the capability to check for support
        /// \return                true if device has this capability
        bool supports_parameter(camera_info info_param) const
        {
            rs_error * e = nullptr;
        ...
        template <typename T>
        PyObject* get_frame_data(stream stream) const
        {
            rs_error * e = nullptr;
            auto r = rs_get_frame_data((const rs_device *)this, (rs_stream)stream, &e);
            error::handle(e);
            //get the dimensions of the stream
            int w = get_stream_width(stream);
            int h = get_stream_height(stream);
            rs::format sizeVal = get_stream_format(stream);
            int size = 1;
            int numElementsPerPixel = 1;

            switch(sizeVal){        
                case rs::format::z16:
                    size = 2;
                    numElementsPerPixel = 1;
                    break;
                ...
                case rs::format::raw10:
                    size = 2;
                    numElementsPerPixel = 1;
                    //this one is messed up "4 10 bit values in 5 byte macro pixel"
                    break;
            ...
            if(size != sizeof(T)){
                //sizes are not the same return array with 1 element
                npy_intp totalElements = 1;
                PyObject* output = PyArray_SimpleNew(1, &totalElements, NPY_UBYTE);
                return output;
            }
            npy_intp totalElements = w*h*numElementsPerPixel;
            PyObject* output;
            int numpyType = NPY_UBYTE;
            if(size == 2){
            ...
            output = PyArray_SimpleNew(1, &totalElements, numpyType);
            memcpy(PyArray_DATA((PyArrayObject *) output), r, totalElements * sizeof(T));
            return output;
        }
    ...
    //     void on_event(rs_log_severity severity, const char * message)
    //     {
