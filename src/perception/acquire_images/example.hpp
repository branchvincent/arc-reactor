...
class texture_buffer
{
    ...
    int fps, num_frames;
    double next_time;
    std::string folder, dev_num;
public:
    void setfolder(std::string s){
        folder = s;
    }

    void setdev(std::string d){
        dev_num = d;
    }
    texture_buffer() : texture(), last_timestamp(-1), fps(), num_frames(), next_time(1000) {write_cnt = 0;}
    ...
    void upload(rs::device & dev, rs::stream stream)
    {
        ...
        if(timestamp != last_timestamp)
        {
            ...
            if(stream == rs::stream::color){
                //save out to file
                cv::Mat rgb_data(dev.get_stream_height(stream), dev.get_stream_width(stream), CV_8UC3, const_cast<void*>(dev.get_frame_data(stream)));
                cv::cvtColor(rgb_data, rgb_data, CV_BGR2RGB);
                cv::imwrite(folder + "/" + dev_num + "_" + std::to_string(write_cnt) + ".png", rgb_data);
                write_cnt++;
            }
