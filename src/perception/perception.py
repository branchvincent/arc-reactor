...
def update_camera_parameters(list_of_serial_nums):
    ...
    for sn in list_of_serial_nums:
        ...
        store.put('/camera/' + sn_to_cam_names[sn] + '/depth/intrinsics', cameraIntrinsics)
        ...
        store.put('/camera/' + sn_to_cam_names[sn] + '/color/intrinsics', cameraIntrinsics)
...
def segment_images(list_of_urls, list_of_bounds, list_of_world_xforms):
    ...
    if len(list_of_urls) != len(list_of_bounds) or len(list_of_bounds) != len(list_of_world_xforms):
        ...
        return
    elif len(list_of_bounds) == 0 or list_of_bounds is None:
    ...
    for i,url in enumerate(list_of_urls):
        ...
        ref_world_xform = list_of_world_xforms[i]

        #crop the image for desired bin 
        intrins = store.get('/camera/' + cam_name + "/depth/intrinsics")
        ...
        if intrins is None or scale is None or coeffs is None or cam_pose_world is None:
            logger.warning("Could not get the intrinsics or depth scale or coeffs for the camera {}. Not segmenting".format(cam_name))
            return
...
if __name__ == "__main__":
    ...
    parser.add_argument('-b', type=str,nargs='+', help="List of bounds")
    parser.add_argument('-x', type=str,nargs='+', help="List of xforms")
