...
class EvaluatePlacementCenter(State):
    ...
    def _handler(self, locations):
        ...
        if idx is not None:
            ...
            container_bottom = min([p[2] for p in container_corners[idx]]) + self.store.get('/planner/ee_local')[2] + self.store.get('/planner/swivel_local')[2]
    ...
    def _build_container_corners(self, location):
        ...
        # c = vectorops.interpolate(local_points[0],local_points[1],0.5)
        # local_points[0] = vectorops.interpolate(local_points[0],c,0.2)
        # local_points[1] = vectorops.interpolate(local_points[1],c,0.2)
