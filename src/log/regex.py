...
from sqlalchemy import String as _String, Text as _Text, event, exc
...
class String(_String):
    """Enhanced version of standard SQLAlchemy's :class:`String`.
...
class Text(_Text):
    """Enhanced version of standard SQLAlchemy's :class:`Text`.

    Supports additional operators that can be used while constructing
    filter expressions.
    """
    class comparator_factory(_Text.comparator_factory):
        """Contains implementation of :class:`Text` operators
        related to regular expressions.
        """
        def regexp(self, other):
            return RegexMatchExpression(self.expr, literal(other), custom_op('~'))
...
class RegexMatchExpression(BinaryExpression):
    """Represents matching of a column against a regular expression."""
...
SQLITE_REGEX_FUNCTIONS = {
    '~': ('REGEXP',
          lambda value, regex: bool(re.search(regex, value))),
    '~*': ('IREGEXP',
           lambda value, regex: bool(re.search(regex, value, re.IGNORECASE))),
    '!~': ('NOT_REGEXP',
           lambda value, regex: not re.search(regex, value)),
    '!~*': ('NOT_IREGEXP',
            lambda value, regex: not re.search(regex, value, re.IGNORECASE)),
}