...
class MyGLViewer(GLSimulationPlugin):
	def __init__(self,world):
		...
		self.start_time=0
		self.packing_mode=2
		self.box_limit=self.world.terrain("order_box").geometry().getBB()
		self.box_limit[0][1]+=0.18
		self.box_limit[1][1]-=0.18
		self.box_limit[0][2]+=0.18
		self.box_limit[1][2]-=0.18
		print self.box_limit[1][2]
		self.score0=[]
		self.score1=[]
		self.h0=[]
		self.h1=[]
	...
	def control_loop(self):
		...
		if self.count>29:
			output_name='test_results_stowing.json'
			with open(output_name,"w") as outfile:
				...
				exit()
		if self.packing_mode==0 and self.sim.getTime()>self.start_time+1+0.1*self.target:
			if self.target<self.world.numRigidObjects():
				...
				self.reset_items()
				self.packing_mode=1
		elif self.packing_mode==1 and self.sim.getTime()>self.start_time+1+0.1*self.target:
			if self.target<self.world.numRigidObjects():
				print "pacing the object!"
	...
	def idle(self):
		...
		if self.flag:
			...
			file_name="../../data/ARC_stowing.xml"
			...
			# print name_index
			order_list=random.sample(xrange(0,40-1),n)
			for i in range(n):
...
if __name__ == "__main__":
	...
	# 	exit()
	world = WorldModel()
	res = world.readFile("../../data/ARC_stowing.xml")
	if not res:
