...
#            return False
    ...
    #TODO    self.clear_milestones()
        #start the vacuum
        self.motion_milestones.append(Milestone(1,self.robot.getConfig(),1,0).get_milestone())
    ...
    def drop_item(self):
        ...
        #turn off the vacuum
        self.motion_milestones.append(Milestone(1,self.self.robot.getConfig(),0,0).get_milestone())
...
#can remove self.motion_milestones reference and have it self.**
    def joint_space_rotate(self,motion_milestones,current_p,target_p,robot,vacuum_status):
        ...
        motion_milestones.append(Milestone(0.05,self.robot.getConfig(),vacuum_status,vacuum_status).get_milestone())
        if delta>0:
            #ccw rotate
            ...
            while t<=steps:
                ...
                q[6]+=dq
                old_q=new_q
                motion_milestones.append(Milestone(t_step,q,vacuum_status,vacuum_status).get_milestone())
            ...
            #     q[1]+=0.05
            #     q[6]+=0.05
            #     self.motion_milestones.append(make_milestone(0.05,q,vacuum_status,vacuum_status))
            #     self.robot.setConfig(q)
            #     delta-=0.05
        else:
            total_t=-delta/max_rotate_speed
    ...
    def add_milestones(self,test_cspace,robot,milestones,t,control_rate,start_T,end_T,vacuum_status,simulation_status,ik_indicator):
        ...
        milestones.append(Milestone(0.05,start_q,vacuum_status,simulation_status).get_milestone())
        while i<=steps:
            ...
            if flag and s and test_cspace.feasible(q):
                ...
                milestones.append(Milestone(t_step,q,vacuum_status,simulation_status).get_milestone())
                i+=1
            else:
    ...
    #        return False
     ...
     #       return False
       ...
       #     return False
        ...
        #turn off the vacuum
        self.motion_milestones.append(Milestone(1,self.robot.getConfig(),0,0).get_milestone())
